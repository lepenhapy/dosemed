...
@app.post("/admin/seed-farmacia-demo")
def seed_farmacia_demo(telefone: str, db: Session = Depends(get_db)):
    tel = validar_telefone(telefone)
    if tel != ADMIN_PHONE:
        raise HTTPException(status_code=403, detail="Acesso restrito.")
    ...
    f = db.query(Farmacia).filter(Farmacia.id == 8).first()
    if not f:
        raise HTTPException(status_code=404, detail="Farmácia 8 não encontrada.")

    f.nome = "Farmácia Vida+"
    f.bairros = "Jd Imperial, Centro, Bosque da Saúde, Setor D"
    f.plano = "basico"
    f.atende_manipulado = 0
    f.rating_total = 63.5
    f.rating_count = 15

    meds = [
        ("Dipirona 500mg",       "Dipirona monoidratada",      "500mg",    0),
        ("Amoxicilina",          "Amoxicilina",                "500mg",    0),
        ("Ibuprofeno",           "Ibuprofeno",                 "600mg",    0),
        ("Paracetamol",          "Paracetamol",                "750mg",    0),
        ("Omeprazol",            "Omeprazol",                  "20mg",     0),
        ("Metformina",           "Metformina",                 "850mg",    0),
        ("Losartana",            "Losartana potássica",        "50mg",     0),
        ("Atorvastatina",        "Atorvastatina cálcica",      "20mg",     0),
        ("Azitromicina",         "Azitromicina",               "500mg",    0),
        ("Nimesulida",           "Nimesulida",                 "100mg",    0),
        ("Captopril",            "Captopril",                  "25mg",     0),
        ("Dexametasona",         "Dexametasona",               "4mg",      0),
        ("Fluoxetina",           "Fluoxetina",                 "20mg",     0),
        ("Clonazepam",           "Clonazepam",                 "2mg",      0),
        ("Amitriptilina",        "Amitriptilina",              "25mg",     0),
        ("Ranitidina",           "Ranitidina",                 "150mg",    0),
        ("Prednisona",           "Prednisona",                 "20mg",     0),
        ("Ciprofloxacino",       "Ciprofloxacino",             "500mg",    0),
        ("Eritromicina",         "Eritromicina",               "500mg",    0),
        ("Budesonida Nasal",     "Budesonida",                 "32mcg",    0),
    ]
    bairros = ["Jd Imperial", "Centro", "Bosque da Saúde", "Setor D", "Jd Universitário"]
    status_pool = (["enviado"] * 5) + (["visualizado"] * 2) + (["convertido"] * 3)

    now = datetime.utcnow()
    db.query(Lead).filter(Lead.farmacia_id == 8).delete()

    for i in range(62):
        dias_atras = 60 - i
        criado = now - timedelta(days=dias_atras, hours=random.randint(0, 10))
        med = random.choice(meds)
        st = random.choice(status_pool)
        lead = Lead(
            farmacia_id=8,
            usuario_bairro=random.choice(bairros),
            medicamento=med[0],
            principio_ativo=med[1],
            miligramas=med[2],
            manipulado=med[3],
            origem="cron",
            status=st,
            preco_cobrado=5.0 if st == "convertido" else None,
            criado_em=criado,
        )
        db.add(lead)

    db.commit()
    return {"ok": True, "mensagem": "Dados de demonstração inseridos para farmácia 8."}
